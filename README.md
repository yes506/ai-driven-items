...
| [`intent-aligner`](skills/intent-aligner/) | 계획 체인의 **상류** 단계 — 계획에 들어가기 전에 사용자의 머릿속에 있는 의도를 끄집어내 명확히 합니다. 대화형 elicitation(소크라테스식 질문 루프 + 5 Whys + 예시/반례 분별)을 통해 사용자가 **만들고 싶은 기능/제품**을 가져왔는지 **겪고 있는 문제/불편함**을 가져왔는지 자동 판별하고, 의도가 수렴할 때까지 반복합니다. 결과물은 두 형식으로 산출됩니다 — `intent.<slug>.md`(구조화된 AI 파싱용 시드) 및 `intent.<slug>.html`(외부 의존성 없는 단일 파일 사람 검증용 문서). 스택·플래너·레인 무관(stack-/planner-/lane-agnostic): 시드의 6개 루브릭 필드(Goal, In-scope features, Out-of-scope, Constraints, Success criteria, Open questions)는 `codebase-planner`가 그대로 읽을 수 있습니다. 다운스트림 `seed-gather-for-plan`이 리서치 자료에서 의도-필터링된 evidence를 모으고, 향후 `plan-establisher` 스킬이 결합된 자료를 `codebase-planner` 루브릭에 맞춰 재형성할 예정입니다. `plan-establisher` 출시 전까지는 `intent.<slug>.md`와 (있다면) `seeds/seed.<intent-slug>.*.md`를 `/codebase-planner`에 바로 전달할 수 있습니다. 모든 mutation은 git worktree 안에서만 일어나며, `confirm merge` 후에만 `dev`로 머지됩니다. 머지 커밋의 `(intent, human-confirmed)` 마커로 `git log`에서 체인이 가시화됩니다. 수동 호출 전용 — `/intent-aligner`. |
| [`seed-gather-for-plan`](skills/seed-gather-for-plan/) | 계획 체인에서 `intent-aligner`의 **하류** 단계. 캡처된 intent(`intent.<slug>.md`)를 받아, 사용자가 제공한 외부 자료(웹 URL, YouTube 링크, PDF, 이미지, 로컬 문서/코드)에서 evidence corpus를 키웁니다. 각 리소스는 WebFetch / yt-dlp / Read로 가져온 뒤 intent의 6개 루브릭(Goal, Constraints, Success criteria, Out-of-scope 등) 렌즈로 필터링되어, 출처 provenance·의도-필터링된 추출·intent 필드별 관련성 설명을 담은 md+html 시드 쌍(`seeds/seed.<intent-slug>.<resource-slug>.{md,html}`)을 리소스당 하나씩 산출합니다. 외부 fetch 직전 `proceed` 게이트가 있어 잘못 붙여넣은 URL은 요청 전에 걸러집니다. 반복 실행 가능 — 각 호출은 자체 git worktree+merge 사이클에서 동작하며 `seeds/`에 누적되고, 3-case 충돌 해소(이전 머지 시드 → suffix, 크래시 복구 → silent overwrite, 진짜 intra-run 충돌 → suffix)로 이전 시드를 보존합니다. 향후 `plan-establisher`에 입력됩니다. 머지 마커 `(seeds, human-confirmed)`로 `git log`에서 체인이 가시화됩니다. 수동 호출 전용 — `/seed-gather-for-plan`. |
