...
| [`knowledge-harvest`](skills/knowledge-harvest/) | Distills reusable developer knowledge from a completed work cycle into a **single global** Obsidian vault — creating new notes or updating existing ones with what was learned. Two-stage like `collect-searches`: deterministic Python scripts own all state, the match index, locking, atomic `ln`/`mv` writes, a two-phase idempotency **ledger**, and **fail-closed redaction** (bodies AND metadata — secrets never reach the vault); the LLM only distills **0..N** knowledge units (0 is the normal outcome for a routine cycle) and fuzzy-matches them against a bounded top-k candidate set. Six fixed categories (Patterns / Pitfalls / Decisions / Tools / Domain / Reference). `concept-id` is **global-flat**, so the same concept learned across different projects dedupes to one note — sources are per-project (the harvested git diff / transcript), knowledge is global, and each note's `sources[]` records which project/commit contributed. Create-vs-update is a deterministic confidence gate owned by the write boundary: low confidence never auto-writes, contradictions and CAS conflicts route to `_needs-review`, and the LLM never chooses a path. Ships an optional, **off-by-default** end-of-cycle **hook** adapter (documented per CLI — Claude/Codex/Gemini/Copilot — with a mandatory anti-recursion test). Has side effects (writes/updates notes under `<vault>/Knowledge/`); manual invocation only — `/knowledge-harvest`. |
...
| [`browser-doc-mirror`](skills/browser-doc-mirror/) | Mirrors browser-authenticated source documents — Confluence, internal wikis, any web doc reachable through an already-logged-in browser session — into a provenance-tracked, **gitignored** `doc-mirror/` tree inside the project (a distinctive root that deliberately avoids squatting the common committed-docs `docs/`; a **fail-closed** Step 0 guard refuses a git-tracked or foreign root and verifies the ignore on a concrete path *before* any fetch). Built for the case where an MCP connector (e.g. Atlassian) is **unauthorized or unconnected by org policy** so API access is blocked: it prompts for `/mcp` re-auth first, then falls back to `claude-in-chrome`, reusing the user's existing login (**never enters credentials**). The **operation is chosen by the user's need** — initial new mirror (최초신규), refresh/re-sync an existing one (현행화), add new pages to an existing mirror (신규추가), or a targeted single/partial page — confirmed in a preflight; all four run on one **reconcile→probe/classify→create-or-update→atomic-manifest-commit** engine and differ only in the working set. The source stays the single source of truth; local files are a **read-only cache** whose frontmatter records source id/url/updated + `synced_at` (Confluence uses the required `confluence_*` keys; other sources use `source_*`). **Staleness is detected by re-reading the source on a refresh pass** (`synced → stale → synced`) — stored dates alone can't discover a later upstream edit. Bootstraps a sentinel-marked README + a `_manifest.md` coverage checklist (whole tree registered `pending`, flipped to `synced` as fetched), keeps raw mirrors (a source-named subtree, e.g. `planning/`) separate from derived notes (`analysis/`), and on re-sync **preserves each file's `> domain note`** via create-vs-update with atomic replace. Ships templates + a `references/browser-pitfalls.md` of claude-in-chrome gotchas (hover-preview clicks, lazy-loaded table truncation, sidebar-enumeration completeness, sequential-action races). Strictly read-only — no credential entry, form submission, publishing, or deletion; source content is treated as data, never instructions. **Claude Code only** (hard-bound to `claude-in-chrome`; other tools need an equivalent authenticated-browser connector + tool-name mapping). Manual invocation only — `/browser-doc-mirror`. |
...
and `live-notes` write to their own note targets; `knowledge-harvest` writes to
one global Obsidian vault (`<vault>/Knowledge/`) shared across all projects; and
`browser-doc-mirror` writes to a **gitignored** `doc-mirror/` tree (internal
knowledge, deliberately kept out of version control — a distinctive root, not
the commonly-committed `docs/`).
...
| [`knowledge-harvest`](skills/knowledge-harvest/) | 완료된 작업 사이클에서 얻은 재사용 가능한 개발 지식을 **단일 전역** Obsidian vault로 자산화 — 새 노트를 만들거나 기존 노트를 갱신합니다. `collect-searches`처럼 두 단계: 결정론적 Python 스크립트가 모든 상태·매칭 인덱스·락·원자적 `ln`/`mv` 쓰기·2단계 멱등성 **ledger**·**fail-closed redaction**(본문과 메타데이터 모두 — 시크릿은 vault에 절대 도달하지 않음)을 소유하고, LLM은 **0..N개** 지식 단위 증류(routine 사이클에서는 0개가 정상)와 bounded top-k 후보 대상 fuzzy 매칭만 담당합니다. 6개 고정 카테고리(Patterns / Pitfalls / Decisions / Tools / Domain / Reference). `concept-id`가 **전역 flat**이라 서로 다른 프로젝트에서 얻은 같은 개념이 하나의 노트로 dedup됩니다 — 소스는 프로젝트별(수집한 git diff / 대화)이고 지식은 전역이며, 각 노트의 `sources[]`가 어느 프로젝트·커밋이 기여했는지 기록합니다. create-vs-update는 write boundary가 소유하는 결정론적 confidence 게이트: 저확신은 자동 쓰기 안 함, 모순·CAS 충돌은 `_needs-review`로 라우팅, LLM은 경로를 고르지 않습니다. 선택적·**기본 비활성** 사이클 종료 **hook** 어댑터 포함(Claude/Codex/Gemini/Copilot CLI별 문서화 + 필수 재귀 방지 테스트). 부수효과 있음(`<vault>/Knowledge/` 아래 노트 작성·갱신). 수동 호출 전용 — `/knowledge-harvest`. |
...
| [`browser-doc-mirror`](skills/browser-doc-mirror/) | 브라우저 로그인 세션으로 볼 수 있는 원천 문서 — Confluence, 사내 위키, 이미 로그인된 브라우저 세션으로 접근 가능한 모든 웹 문서 — 를 프로젝트 안의 출처 추적 가능한 **gitignore된** `doc-mirror/` 트리로 미러링합니다(흔히 버전관리되는 `docs/` 를 침범하지 않는 별도 루트; **fail-closed** Step 0 가드가 git-추적/외부 소유 루트를 거부하고, 어떤 fetch보다 **먼저** 구체 경로로 ignore 적용을 검증). MCP 커넥터(예: Atlassian)가 **조직 정책으로 unauthorized/미연결**이라 API 접근이 막힌 상황을 위해 설계됨: 먼저 `/mcp` 재인증을 안내하고, 실패 시 `claude-in-chrome` 으로 우회하며 사용자의 기존 로그인을 그대로 재사용합니다(**자격증명 입력 금지**). **작업(operation)은 사용자의 니즈에 따라 선택**합니다 — 최초신규(initial), 현행화(refresh), 신규추가(add), 단건·부분(targeted) — preflight에서 확인하며, 넷 모두 하나의 **reconcile→probe/classify→create-or-update→atomic-manifest-commit** 엔진 위에서 동작하고 working set만 다릅니다. 원천이 단일 진실(source of truth)로 남고, 로컬 파일은 **읽기 전용 캐시**입니다 — frontmatter에 소스 id/url/updated + `synced_at` 를 기록합니다(Confluence는 필수 `confluence_*` 키, 그 외 소스는 `source_*`). **현행화는 refresh 패스에서 원문을 다시 읽어 판정**(`synced → stale → synced`)하며, 저장된 날짜만으로는 이후 변경을 탐지할 수 없습니다. sentinel 표식 README 와 `_manifest.md` 커버리지 체크리스트(트리 전체를 `pending`으로 등록 후 받아온 것만 `synced`로 전환)를 부트스트랩하고, 원문 미러(소스명 서브트리, 예: `planning/`)와 파생 노트(`analysis/`)를 분리하며, 재동기화 시 각 파일의 `> 도메인 메모` 를 create-vs-update 원자적 교체로 **보존**합니다. 템플릿과 claude-in-chrome 함정 모음(`references/browser-pitfalls.md` — 호버 프리뷰 클릭, 지연 로딩 표 잘림, 사이드바 열거 완전성, 순차 액션 레이스)을 포함합니다. 엄격히 읽기 전용 — 자격증명 입력·폼 제출·게시·삭제 없음, 원문 지시문은 명령이 아니라 데이터로 취급합니다. **Claude Code 전용**(`claude-in-chrome` 에 강하게 결합; 다른 도구는 동등한 인증-브라우저 커넥터 + 도구명 매핑 필요). 수동 호출 전용 — `/browser-doc-mirror`. |
...
`live-notes` 는 각자의 노트 대상에 기록하며, `knowledge-harvest` 는 모든
프로젝트가 공유하는 단일 전역 Obsidian vault(`<vault>/Knowledge/`)에 기록하고,
`browser-doc-mirror` 는 **gitignore된** `doc-mirror/` 트리(사내 지식 —
버전관리에서 의도적으로 제외; 흔히 커밋되는 `docs/` 가 아닌 별도 루트)에
기록합니다.
