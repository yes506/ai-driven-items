...
Codex CLI has a near-identical skill convention.
...
# Option B — paste the SKILL.md body into the project's GEMINI.md
cp skills/project-scaffolder/SKILL.md ./GEMINI.md   # then edit out the YAML frontmatter
```
...
For the `gh copilot` CLI specifically (one-shot prompts), copy/paste the SKILL.md body into the prompt or pipe it in:

```bash
gh copilot suggest "$(cat skills/project-scaffolder/SKILL.md) — now scaffold a Next.js app"
```
...
---
EOF
# Append the SKILL.md body (everything after the frontmatter) to the rule file:
sed -n '/^---$/,/^---$/!p' skills/project-scaffolder/SKILL.md >> .cursor/rules/project-scaffolder.mdc
```
...
# 옵션 B — SKILL.md 본문을 프로젝트의 GEMINI.md 에 붙여넣기
cp skills/project-scaffolder/SKILL.md ./GEMINI.md   # 이후 YAML frontmatter 제거
```
...
`gh copilot` CLI(원샷 프롬프트)의 경우, SKILL.md 본문을 프롬프트에 복붙하거나 파이프로 넘기세요:

```bash
gh copilot suggest "$(cat skills/project-scaffolder/SKILL.md) — Next.js 앱을 스캐폴딩해줘"
```
...
---
EOF
# SKILL.md 본문(frontmatter 이후) 을 룰 파일에 이어붙임:
sed -n '/^---$/,/^---$/!p' skills/project-scaffolder/SKILL.md >> .cursor/rules/project-scaffolder.mdc
```
