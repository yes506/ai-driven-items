...
#   inside-seed-worktree      — cwd is inside .worktrees/seed-* (resume target)
#   inside-other-worktree     — inside a non-seed linked worktree (intent,
#                               planner, implementer, scaffold, unknown — refuse)
...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  # Route through build_json so the contract holds: schema docstring at
  # the top of this file declares `main_checkout`/`default_branch`/etc.
  # as "<absolute path or empty>" strings — always a string, never null.
  MAIN_CHECKOUT="" DEFAULT_BRANCH="" CURRENT_BRANCH="" DEV_EXISTS=false
  emit "$(build_json unrelated not_in_git_repo)"
fi
...
if [ "${IS_LINKED_WORKTREE}" = "true" ]; then
  case "${WORKTREE_TOP}" in
    */.worktrees/seed-*)   emit "$(build_json inside-seed-worktree)" ;;
