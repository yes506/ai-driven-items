...
planner handoff text) are NOT rendered — those stay in intent.<slug>.md.
...
_COUNTER_RE = re.compile(r"\s*\(counter-example:\s*(.+?)\)\s*$", re.IGNORECASE)


def _split_counter_reason(item: str):
    """Split an out-of-scope entry on '(counter-example: <reason>)'.

    Returns (head, reason_or_None). Falls back to (item, None) when the
    pattern isn't present. See references/output-schema.md for the fold.
    """
    if not isinstance(item, str):
        return str(item), None
    m = _COUNTER_RE.search(item)
    if not m:
        return item.strip(), None
    head = item[: m.start()].rstrip()
...
def _scope_block(intent: dict, t: dict) -> str:
    ...
    def _out_li(item):
        head, reason = _split_counter_reason(item)
        if reason:
            return (
                f'<li>{_esc(head)}'
        ...
        return f"<li>{_esc(head)}</li>"

    in_html = (
        f'<ul>{"".join(_in_li(x) for x in in_scope)}</ul>'
        if in_scope else f'<p class="placeholder">{_esc(t["not_recorded"])}</p>'
    )
    out_html = (
        f'<ul>{"".join(_out_li(x) for x in out_scope)}</ul>'
...
def _root_cause_block(intent: dict, mode: str, t: dict) -> str:
    ...
    Suppressed for n<2 chains: a one-element "chain" has no flow, and the
    constraint-list fold (`Root cause: <X>` per the schema) already
    carries the single step. Avoids labeling a lone step "Symptom".
    """
    if mode != "problem":
        return ""
...
def _examples_block(intent: dict, t: dict) -> str:
    ...
    return (
        ...
        f'<div class="ex-col good"><h2>{_esc(t["examples_good"])}</h2>'
        f'{_items(good, t["no_examples"])}</div>'
        f'<div class="ex-col bad"><h2>{_esc(t["examples_bad"])}</h2>'
...
def main() -> int:
    ...
    replacements = {
        ...
        "EXAMPLES_BLOCK":       _examples_block(intent, t),
