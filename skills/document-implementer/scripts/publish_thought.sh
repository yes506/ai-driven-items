#!/usr/bin/env bash
# publish_thought.sh — publish a document-implementer thinking-checkpoint to
# canvas-terminal shared collab-memory so peer agents can read along.
#
# Usage:
#   publish_thought.sh <docimpl_id> <phase> <topic> [< body-on-stdin]
#
...
if [ "$#" -lt 3 ]; then
  printf 'usage: %s <docimpl_id> <phase> <topic> [< body]\n' "$0" >&2
  exit 2
fi
...
OUT="${SESSION_DIR}/docimpl-${ID_SAFE}-phase-${PHASE_SAFE}-${TOPIC_SAFE}.md"
TMP="${OUT}.tmp.$$"
...

{
  ...
  printf -- 'topic: %s\n' "${TOPIC_SAFE}"
  printf -- 'ts: %s\n' "${NOW_ISO}"
  printf -- 'author: document-implementer\n'
  printf -- '---\n\n'
  ...
  printf -- '`docimpl-%s-feedback-<your-handle>-<topic>.md`_._\n' "${ID_SAFE}"
} > "${TMP}"
