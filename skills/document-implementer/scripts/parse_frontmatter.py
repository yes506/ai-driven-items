...
MIRROR DISCIPLINE: this file is a COPY of
`skills/document-planner/scripts/parse_frontmatter.py`. The planner-side
file is canonical. On any divergence, the planner copy wins;
document-implementer is bumped to track. Update the planner-side first,
then mirror here. Drift is caught at review time (same posture as
`references/marker-detection.md`) — there is no runtime hash check.
Reviewers must diff the two copies during any patch that touches either.
