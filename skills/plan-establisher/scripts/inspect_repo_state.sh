...
#   inside-plan-worktree      — cwd is inside .worktrees/plan-* (resume target)
#   inside-other-worktree     — inside a non-plan linked worktree (intent,
#                               seed, planner, implementer, scaffold,
#                               unknown — refuse)
...
if [ "${IS_LINKED_WORKTREE}" = "true" ]; then
  case "${WORKTREE_TOP}" in
    */.worktrees/plan-*)   emit "$(build_json inside-plan-worktree)" ;;
